    ...
    extends Specification
{
  def 'it redirects to the application composition report'() {
    setup:
      ...
      def healthAction = new PolicyEvaluationHealthAction(null, 'appId', 'stage', null, policyEvaluation)
  ...
  def 'it returns no project actions before build'() {
    setup:
      def healthAction = new PolicyEvaluationHealthAction(null, 'appId', 'stage', null, Mock(ApplicationPolicyEvaluation))
  ...
  def 'it returns policy project action of builds parents'() {
    setup:
      ...
      def job = Mock(Job)
      run.getParent() >> job
      def healthAction = new PolicyEvaluationHealthAction(null, 'appId', 'stage', run, Mock(ApplicationPolicyEvaluation))
  ...
  def 'it returns the correct component and grandfathered policy violation counts'() {
    setup:
      ...
      def healthAction = new PolicyEvaluationHealthAction(null, 'my-iq-app', 'build', null, policyEvaluation)
      def response = Mock(StaplerResponse)
  ...
  def 'health action and project action have matching report links'() {
    setup:
      ...
      def run = Mock(Run)
      def policyEvaluation = new ApplicationPolicyEvaluation(1, 2, 3, 4, 11, 12, 13, 5, [], reportLink)
      def healthAction = new PolicyEvaluationHealthAction(null, 'appId', 'stage', run, policyEvaluation)
